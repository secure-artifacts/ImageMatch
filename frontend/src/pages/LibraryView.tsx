...
export default function LibraryView({ onLibraryChange }: LibraryViewProps) {
    ...
    for (const item of queue) {
      ...
      for (let i = 0; i < files.length; i += BATCH_SIZE) {
        ...
        setFolderQueue((prev) =>
          prev.map((q) =>
            q.id === item.id
              ? { ...q, uploaded, failed, progress: Math.min(progress, 100) }
              : q
          )
        );
      }
  ...
  }, [isProcessingQueue, fetchImages, onLibraryChange]);
            ...
            {folderQueue.map((item) => (
              ...
              >
                ...
                >
                  ...
                  <span className="text-xs text-surface-500 shrink-0">
                    ...
                    {item.status === 'done' && (
                      <span className="text-green-400">
                        {item.uploaded} 张完成{item.failed > 0 ? `，${item.failed} 张失败` : ''}
                      </span>
                    )}
                  ...
                  {item.expanded
                    ? <ChevronUp className="w-3.5 h-3.5 text-surface-600 shrink-0" />
                    : <ChevronDown className="w-3.5 h-3.5 text-surface-600 shrink-0" />
                  }
                </div>
                      ...
                      <div className="mt-1">
                        <div className="w-full h-1.5 bg-surface-800 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all duration-500 ${
                              item.status === 'done' ? 'bg-green-500' : 'upload-progress-bar'
                            }`}
                        ...
                        <div className="flex justify-between mt-1.5">
                          <span className="text-xs text-surface-600">
                            {item.status === 'uploading' ? `使用 ResNet50 提取特征向量中...` : '✓ 全部处理完成'}
                          </span>
                          <span className={`text-xs font-medium ${item.status === 'done' ? 'text-green-400' : 'text-purple-400'}`}>
                            {item.progress}%
                          </span>
                        </div>
                      </div>
